...
export class PushDb {
  static supabase: DbClient | null = null;
  ...
  static init({
    ...
    if (!this.supabase) {
      this.supabase = createClient(url, WIBU_PUSH_DB_TOKEN);
    }
  ...
  static async create({ id, data }: { id?: string; data: any }) {
  ...
  static async upsert({ id, data }: { id?: string; data: any }) {
    if (!this.supabase || !this.project) {
      throw new Error("Realtime client or project not initialized.");
    }
